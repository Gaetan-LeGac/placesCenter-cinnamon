...
function IconMenuItem(text, icon, forceFM){
    this._init(text, icon, forceFM);
}
...
function VolumeMenuItem(volume, mounted, forceFM) {
    this._init(volume, mounted, forceFM);
}

VolumeMenuItem.prototype = {
    __proto__: IconMenuItem.prototype,

    _init: function(volume, mounted, forceFM) {
		this.forceFM = forceFM;
        ...
        if ( mounted ) {
            ...
            this.connect("activate", function() {
				if (this.forceFM)
					Main.Util.spawnCommandLine(this.forceFM+" "+volume.get_mount().get_root().get_uri());
				else
...
function PlaceMenuItem(text, uri, iconName, forceFM) {
    this._init(text, uri, iconName, forceFM);
}

PlaceMenuItem.prototype = {
    __proto__: FolderTypeMenuItem.prototype,

    _init: function(uri, text, iconName, forceFM) {
        this.uri = uri;
		this.forceFM = forceFM;
    ...
    launch: function(event) {
		if (this.forceFM)
			Main.Util.spawnCommandLine(this.forceFM+" "+this.uri);
		else
        	Gio.app_info_launch_default_for_uri(this.uri, global.create_app_launch_context());
    }
}

function FTPMenuItem(text, uri, forceFM) {
    this._init(text, uri, forceFM);
}
FTPMenuItem.prototype = {

    __proto__: FolderTypeMenuItem.prototype,

	forceFM: null,

    _init: function(uri, text, forceFM) {
        this.uri = uri;
	    this.forceFM = forceFM;

		let icon = new St.Icon({icon_name: "server", icon_size: menu_item_icon_size, icon_type: St.IconType.FULLCOLOR});
        FolderTypeMenuItem.prototype._init.call(this, text, icon);
...
function SSHMenuItem(text, uri) {
    this._init(text, uri);
}

SSHMenuItem.prototype = {
    __proto__: FolderTypeMenuItem.prototype,

	forceFM: null,

    _init: function(uri, text) {
        this.uri = uri;

		let icon = new St.Icon({icon_name: "terminal", icon_size: menu_item_icon_size, icon_type: St.IconType.FULLCOLOR});
        FolderTypeMenuItem.prototype._init.call(this, text, icon);
    ...
    launch: function(event) {
		Main.Util.spawnCommandLine("gnome-terminal -e 'ssh "+this.uri+"'");
    }
}
...
MyApplet.prototype = {
    ...
    _onButtonPressEvent: function(actor, event) {
        if ( event.get_button() == 2 ) {
            ...
            if ( uri ) {
				if (this.forceFM)
					Main.Util.spawnCommandLine(this.forceFM+" "+uri);
				else
    ...
    bindSettings: function() {
        ...
        this.settings.bindProperty(Settings.BindingDirection.IN, "forceFM", "forceFM", this.buildMenu);
        ...
        this.settings.bindProperty(Settings.BindingDirection.IN, "sftpList", "sftpList", this.buildDistantSection);
        this.settings.bindProperty(Settings.BindingDirection.IN, "sshList", "sshList", this.buildDistantSection);
    ...
    buildMenu: function() {
        try {
            ...
            let distantPaneBox = new St.BoxLayout({ style_class: "xCenter-pane" });
            let distantPane = new PopupMenu.PopupMenuSection();
            distantPaneBox.add_actor(distantPane.actor);
            let distantTitle = new PopupMenu.PopupBaseMenuItem({ style_class: "xCenter-title", reactive: false });
            distantPane.addMenuItem(distantTitle);
            distantTitle.addActor(new St.Label({ text: _("DISTANT") }));
            section._connectSubMenuSignals(distantPane, distantPane);

			this.distantSection = new PopupMenu.PopupMenuSection();
            distantPane.addMenuItem(this.distantSection);

			mainBox.add_actor(distantPaneBox);
            this.buildDistantSection();
    ...
    buildUserSection: function() {
        ...
        for ( let bookmark of bookmarks ) {
        	global.log(bookmark);
            let bookmarkItem = new FolderTypeMenuItem(bookmark.name, bookmark.iconFactory(menu_item_icon_size), this.forceFM);
            this.userSection.addMenuItem(bookmarkItem);
            let launch = bookmark.launch;
            bookmarkItem.connect("activate", Lang.bind(this, function() {
				// TODO: Force File manager
				// Problem: How to get bookmark uri ?
                launch();
            }));
        }
    ...
    buildSystemSection: function() {
        ...
        if ( this.showComputer ) {
            let computer = new PlaceMenuItem("computer:///", _("Computer"), "computer", this.forceFM);
            this.systemSection.addMenuItem(computer);
        }

        //file system
        if ( this.showRoot) {
            let fileSystem = new PlaceMenuItem("file:///", _("File System"), "drive-harddisk", this.forceFM);
        ...
        //network items
        if ( this.showNetwork ) {
            let network = new PlaceMenuItem("network:///", _("Network"), "network-workgroup", this.forceFM);
            this.systemSection.addMenuItem(network);

            let bookmark = Main.placesManager.getDefaultPlaces()[2];
            let connectToItem = new IconMenuItem(bookmark.name, bookmark.iconFactory(menu_item_icon_size), this.forceFM);
        ...
        for ( let i = 0; i < customPlaces.length; i++ ) {
            ...
            try {
                ...
                if ( file.query_exists(null) ) {
                    ...
                    customPlace = new PlaceMenuItem(place, text, iconName, this.forceFM);
    ...
    buildDistantPlaces: function(list, container) {
        if ( list == "" ) return;
        let distantPlace;
        let distantPlaces = list.split(/, *|\n/);

        for ( let i = 0; i < distantPlaces.length; i++ ) {
            if ( distantPlaces[i] == "" ) continue;
            try {
                let entry = distantPlaces[i].split("=");
				let protocol = entry[1].split(":")[0].trim();

				switch (protocol) {
					case "ftp":
					case "sftp":
						distantPlace = new FTPMenuItem(entry[1].trim(), entry[0].trim(), this.forceFM);
						break;
					default:
						distantPlace = new SSHMenuItem(entry[1].trim(), entry[0].trim());
						break;
				}

				container.addMenuItem(distantPlace);
    ...
    buildDevicesSection: function() {
        ...
        for ( let volume of volumes ) {
            ...
            let volumeMenuItem = new VolumeMenuItem(volume, mounted, this.forceFM);
    ...
    buildRecentDocumentsSection: function() {
        ...
        for ( let i = 0; i < showCount; i++ ) {
            ...
            let recentItem = new IconMenuItem(recentInfo.get_display_name(), mimeType, this.forceFM);
            this.recentSection.addMenuItem(recentItem);
            recentItem.connect("activate", Lang.bind(this, function() {
				if (this.forceFM)
					Main.Util.spawnCommandLine(this.forceFM+" "+recentInfo.get_uri());
				else
    ...
    buildDistantSection: function() {
        this.distantSection.removeAll();

        //custom places
        this.buildDistantPlaces(this.sftpList, this.distantSection);
        this.buildDistantPlaces(this.sshList, this.distantSection);
    },
    ...
    buildTrashItem: function() {
        ...
        this.trashItem = new PlaceMenuItem(uri, _("Trash"), iName, this.forceFM);
